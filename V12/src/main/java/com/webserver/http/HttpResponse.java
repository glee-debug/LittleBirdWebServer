...
//            }
    ...
    public void setEntity(File file) {
        ...
        putHeaders("Content-Type", HttpContext.getMime(key));
        putHeaders("Content-Length",String.valueOf(file.length()));
        putHeaders("Server","LittleBirdWebServer");
    }

    public String parsePath(String path){
        int last = path.lastIndexOf(".")+1;
        String fileType = path.substring(last,path.length());
        if (!HttpContext.mime.keySet().contains(fileType)) {
